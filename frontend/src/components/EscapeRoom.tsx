...
const EscapeRoom = () => {
  // 1️⃣  Read the logged-in user from context
  const { user } = useContext(AuthContext) as any;
  const navigate = useNavigate();

  // 2️⃣  Door animation: hidden → sliding → shown
  const [phase, setPhase] = useState<"hidden" | "sliding" | "shown">("hidden");

  useEffect(() => {
    if (user) {
      setTimeout(() => setPhase("sliding"), 100);  // start sliding
      setTimeout(() => setPhase("shown"),   1800); // fully open
    }
  }, [user]);

  // 3️⃣  Canvas — animated dot grid + floating particles
  ...
  useEffect(() => {
    ...
    // Create 30 random floating particles
    ...
    const draw = () => {
      ...
      // Particles — drift upward, wrap when they leave the top
      particles.forEach(p => {
        p.x += p.vx; p.y += p.vy;
  ...
  // ─── Render ──────────────────────────────────────────────────────────────
  return (
    <div style={{ minHeight:"100vh", background:"#0c1120", position:"relative", overflow:"hidden", fontFamily:"'JetBrains Mono','Fira Code',monospace" }}>
      <style>{`
      ...
      <div style={{ position:"fixed", inset:0, zIndex:1, pointerEvents:"none", overflow:"hidden" }}>
        <div style={{ position:"absolute", left:0, right:0, height:"80px", background:"linear-gradient(180deg, transparent, rgba(97,218,251,0.02), transparent)", animation:"scanline 7s linear infinite" }} />
      </div>

      {/* Top accent bar */}
      <div style={{ position:"fixed", top:0, left:0, right:0, height:"2px", zIndex:100, background:"linear-gradient(90deg, rgba(97,218,251,0.55), rgba(167,139,250,0.45), rgba(247,223,30,0.4), rgba(255,107,107,0.45))" }} />

      {/* Show login form if not logged in */}
      {!user && <LoginForm onLoginSuccess={() => {}} />}

      {/* ── Sliding door panels (appear after login) ── */}
      {user && phase !== "hidden" && (
        <>
          {/* Left door */}
          <div style={{ position:"fixed", top:0, left:0, width:"50%", height:"100%", background:"linear-gradient(90deg, #0c1120 0%, #111827 100%)", zIndex:50, transform: phase !== "hidden" ? "translateX(-100%)" : "translateX(0)", transition:"transform 1.4s cubic-bezier(0.77, 0, 0.18, 1) 0.1s", borderRight:"1px solid rgba(97,218,251,0.15)" }}>
            <div style={{ position:"absolute", inset:0, backgroundImage:"repeating-linear-gradient(180deg, transparent 0px, rgba(97,218,251,0.015) 1px, transparent 2px)" }} />
          </div>
          {/* Right door */}
          <div style={{ position:"fixed", top:0, right:0, width:"50%", height:"100%", background:"linear-gradient(270deg, #0c1120 0%, #111827 100%)", zIndex:50, transform: phase !== "hidden" ? "translateX(100%)" : "translateX(0)", transition:"transform 1.4s cubic-bezier(0.77, 0, 0.18, 1) 0.1s", borderLeft:"1px solid rgba(97,218,251,0.15)" }}>
            <div style={{ position:"absolute", inset:0, backgroundImage:"repeating-linear-gradient(180deg, transparent 0px, rgba(97,218,251,0.015) 1px, transparent 2px)" }} />
          </div>
        </>
      )}

      {/* ── Welcome content (visible after doors slide open) ── */}
      {user && (
          ...
          <div style={{ maxWidth:"640px", width:"100%", textAlign:"center" }}>
            ...
            <div style={{ background:"rgba(14,20,36,0.6)", border:"1px solid rgba(97,218,251,0.09)", borderRadius:"10px", padding:"20px 22px", textAlign:"left", marginBottom:"24px", animation:"fadeUp 0.6s 0.28s ease both" }}>
              ...
              <div style={{ display:"flex", gap:"14px", marginBottom:"14px", alignItems:"flex-start" }}>
                ...
                <div>
                  <p style={{ fontFamily:"'Inter',sans-serif", fontSize:"12px", color:"rgba(180,210,230,0.6)", margin:"0 0 3px 0", lineHeight:"1.5" }}>
                    Open <strong style={{ color:"rgba(180,210,230,0.85)" }}>DevTools</strong> → Application → Local Storage
                  </p>
                  <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"10px", color:"rgba(97,218,251,0.35)", margin:0 }}>
                    Find the <span style={{ color:"rgba(97,218,251,0.55)" }}>token</span> key — that's your JWT
                  </p>
                </div>
              </div>
              ...
              <div style={{ display:"flex", gap:"14px", alignItems:"flex-start" }}>
                ...
                <div>
                  <p style={{ fontFamily:"'Inter',sans-serif", fontSize:"12px", color:"rgba(180,210,230,0.6)", margin:"0 0 3px 0", lineHeight:"1.5" }}>
                    ...
                    <a href="https://jwt.io" target="_blank" rel="noopener noreferrer" style={{ color:"rgba(97,218,251,0.7)", fontFamily:"'JetBrains Mono',monospace", fontSize:"12px", textDecoration:"none", borderBottom:"1px solid rgba(97,218,251,0.25)" }}>jwt.io ↗</a>
                  </p>
                  <p style={{ fontFamily:"'Inter',sans-serif", fontSize:"11px", color:"rgba(247,223,30,0.4)", margin:0, lineHeight:"1.5" }}>
                    The payload is fully readable — it's Base64, not encrypted.{" "}
            ...
            {/* 5️⃣  Context state — shows what changed after login */}
            <div style={{ background:"rgba(16,22,40,0.55)", border:"1px solid rgba(167,139,250,0.09)", borderRadius:"10px", padding:"18px 22px", textAlign:"left", marginBottom:"24px", animation:"fadeUp 0.6s 0.33s ease both" }}>
              <p style={{ fontSize:"10px", color:"rgba(167,139,250,0.3)", letterSpacing:"0.12em", marginBottom:"12px" }}>// react context state — all subscribed components re-rendered</p>
            ...
            <div style={{ display:"flex", gap:"12px", justifyContent:"center", animation:"fadeUp 0.6s 0.38s ease both" }}>
              <button className="badge-btn" onClick={() => navigate("/game")}
                style={{ padding:"12px 22px", fontSize:"12px", fontWeight:"600", fontFamily:"'JetBrains Mono',monospace", letterSpacing:"0.06em", color:"#0c1120", background:"linear-gradient(135deg, rgba(97,218,251,0.75), rgba(58,184,216,0.75))", border:"none", borderRadius:"6px", cursor:"pointer", boxShadow:"0 4px 18px rgba(97,218,251,0.12)" }}>
                ← Back to Login
              </button>
              <a href="https://jwt.io" target="_blank" rel="noopener noreferrer" className="badge-btn"
                style={{ padding:"12px 22px", fontSize:"12px", fontWeight:"600", fontFamily:"'JetBrains Mono',monospace", letterSpacing:"0.06em", color:"#61dafb", background:"rgba(97,218,251,0.06)", border:"1px solid rgba(97,218,251,0.2)", borderRadius:"6px", cursor:"pointer", textDecoration:"none", display:"inline-flex", alignItems:"center", gap:"6px" }}>
                ↗ jwt.io
              </a>
            </div>
