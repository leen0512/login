...
// Soft muted palette — slate/indigo tones, no neon
const C = {
  bg: "#1a1f2e",           // soft navy, not pitch black
  panel: "rgba(30,36,52,0.8)",
  border: "rgba(255,255,255,0.07)",
  text: "#c4cdd8",
  textMuted: "rgba(148,165,185,0.5)",
  // JWT segment colors — muted, easy on eyes
...
const LoginForm = ({ onLoginSuccess }: { onLoginSuccess?: () => void }) => {
  ...
  const handleLogin = async () => {
    setIsLoading(true); setFlowStep(3);
    try {
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-main {
          ...
          background: rgba(110,185,210,0.18);
          color: rgba(129,200,220,0.9);
          border: 1px solid rgba(110,185,210,0.2);
          transition: all 0.25s ease;
          display: flex; align-items: center; justify-content: center; gap: 8px;
        }
        .btn-main:hover:not(:disabled) {
          background: rgba(110,185,210,0.26);
          border-color: rgba(110,185,210,0.4);
          color: rgba(170,220,235,1);
        ...
        /* JWT chips */
        .jwt-chip {
          flex: 1; padding: 9px 12px; border-radius: 6px; text-align: center;
      ...
      {/* ── LEFT PANEL ── */}
      <div style={{
        width: "390px", minHeight: "100vh", flexShrink: 0,
        background: "rgba(20,25,38,0.96)",
        borderRight: `1px solid ${C.border}`,
        display: "flex", flexDirection: "column", justifyContent: "center",
        padding: "44px 34px", position: "relative", zIndex: 10,
        animation: "fadeUp 0.6s ease both",
      }}>
        {/* Subtle top gradient line */}
        <div style={{
          position:"absolute", top:0, left:0, right:0, height:"1px",
          background:"linear-gradient(90deg, transparent, rgba(110,185,210,0.3), rgba(150,125,210,0.25), transparent)",
        }} />
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"28px" }}>
          ...
          <div>
            <div style={{ fontSize:"12px", fontWeight:"600", color:"rgba(129,200,220,0.7)", letterSpacing:"0.04em" }}>AuthContext Lab</div>
            <div style={{ fontSize:"10px", color:"rgba(148,165,185,0.4)", letterSpacing:"0.07em" }}>Workshop Demo · סדנה 2</div>
          </div>
          <span style={{
            marginLeft:"auto", fontSize:"9px", color:"rgba(110,185,210,0.45)",
            background:"rgba(110,185,210,0.06)", border:"1px solid rgba(110,185,210,0.12)",
            borderRadius:"3px", padding:"2px 7px", letterSpacing:"0.1em",
          }}>LIVE</span>
        </div>

        <h1 style={{ fontSize:"22px", fontWeight:"700", color:"#c4cdd8", letterSpacing:"-0.01em", marginBottom:"4px" }}>Sign In</h1>
        <p style={{ fontFamily:"'Inter',sans-serif", fontSize:"12px", color:"rgba(148,165,185,0.45)", marginBottom:"28px" }}>
          React Auth Context &amp; JWT
        </p>

        {/* Username */}
        <div style={{ marginBottom:"16px" }}>
          <label style={{
            display:"block", fontSize:"10px", letterSpacing:"0.1em", marginBottom:"6px",
            color: focusedField==="u" ? "rgba(110,185,210,0.6)" : "rgba(148,165,185,0.35)",
        ...
        <div style={{ marginBottom:"22px" }}>
          <label style={{
            display:"block", fontSize:"10px", letterSpacing:"0.1em", marginBottom:"6px",
            color: focusedField==="p" ? "rgba(110,185,210,0.6)" : "rgba(148,165,185,0.35)",
            transition:"color 0.25s",
          }}>// password</label>
          ...
          <p style={{ fontFamily:"'Inter',sans-serif", fontSize:"10px", color:"rgba(190,120,110,0.45)", marginTop:"5px" }}>
        ...
        <button className="btn-main" onClick={handleLogin} disabled={isLoading || !username || !password}>
          {isLoading ? (
            <>
              <span style={{ width:"12px", height:"12px", border:"1.5px solid rgba(110,185,210,0.2)", borderTop:"1.5px solid rgba(110,185,210,0.8)", borderRadius:"50%", display:"inline-block", animation:"spin 0.7s linear infinite" }} />
              Authenticating...
            </>
          ) : "→  POST /api/login"}
        </button>
        ...
        <div style={{
          ...
          <p style={{ fontFamily:"'Inter',sans-serif", fontSize:"11px", color:"rgba(175,155,100,0.5)", lineHeight:"1.7" }}>
            ...
            <a href="https://jwt.io" target="_blank" rel="noopener noreferrer"
              style={{ color:"rgba(110,185,210,0.6)", textDecoration:"none", borderBottom:"1px solid rgba(110,185,210,0.2)" }}>
              jwt.io
            </a>
        ...
        <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"10px", color:"rgba(148,165,185,0.18)", marginTop:"18px", textAlign:"center", fontStyle:"italic" }}>
          <span style={{ animation:"blink 1.2s ease-in-out infinite", display:"inline-block" }}>_</span>
          {" "}type to activate flow →
        </p>
      </div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"36px 44px", position:"relative", zIndex:10, minWidth:0, overflowY:"auto" }}>
        ...
        <div style={{ marginBottom:"24px", animation:"fadeUp 0.6s 0.08s ease both" }}>
          ...
          <h2 style={{ fontSize:"17px", fontWeight:"700", color:"#c4cdd8", letterSpacing:"-0.01em" }}>From Login → Context Update</h2>
          <p style={{ fontFamily:"'Inter',sans-serif", fontSize:"12px", color:"rgba(148,165,185,0.35)", marginTop:"4px" }}>
            Start typing — watch the flow light up
          </p>
        </div>
            ...
            {flowNodes.map((node, i) => {
              ...
              return (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:"4px", flexShrink:0 }}>
                  <div style={{
                    padding:"9px 12px", borderRadius:"7px", textAlign:"center", minWidth:"82px",
                    background: active ? `${color.replace("0.7","0.08")}` : "rgba(255,255,255,0.02)",
                    border: `1px solid ${active ? color.replace("0.7","0.35") : "rgba(255,255,255,0.05)"}`,
                  ...
                  {i < flowNodes.length - 1 && (
                    <div style={{ width:"18px", height:"1px", borderRadius:"1px", position:"relative", background: flowStep > i ? color.replace("0.7","0.4") : "rgba(255,255,255,0.07)", overflow:"hidden", flexShrink:0, transition:"background 0.5s" }}>
        ...
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:"10px", padding:"20px 18px", marginBottom:"16px", animation:"fadeUp 0.6s 0.16s ease both" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"12px" }}>
            ...
            <a href="https://jwt.io" target="_blank" rel="noopener noreferrer" style={{
              fontFamily:"'JetBrains Mono',monospace", fontSize:"10px",
              color:"rgba(110,185,210,0.5)", textDecoration:"none",
              background:"rgba(110,185,210,0.06)", border:"1px solid rgba(110,185,210,0.15)",
              borderRadius:"4px", padding:"2px 9px",
            }}>↗ jwt.io</a>
          </div>

          {/* Token string — segments colored, active one lit */}
          <div style={{
            padding:"11px 13px", background:"rgba(0,0,0,0.2)", borderRadius:"6px",
            border:`1px solid ${C.border}`, marginBottom:"10px",
            fontSize:"11px", wordBreak:"break-all", lineHeight:"1.8", userSelect:"text",
          }}>
            {jwtParts.map((part, i) => {
              const seg = jwtSegmentInfo[i];
              ...
              return (
                <span key={i}>
                  <span
                    ...
                    style={{
                      color: isActive ? seg.c.lit : seg.c.base,
                      background: isActive ? seg.c.bg : "transparent",
                      borderRadius:"3px", padding:"1px 2px",
                      cursor:"pointer",
                      transition:"color 0.2s, background 0.2s",
                      textDecoration: isActive ? "underline" : "none",
                      textDecorationColor: seg.c.lit,
            ...
            {jwtSegmentInfo.map((seg, i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    flex:1, padding:"9px 12px", borderRadius:"6px", textAlign:"center",
                    cursor:"pointer",
                    background: isActive ? seg.c.bg : "rgba(255,255,255,0.02)",
                    border: `1px solid ${isActive ? seg.c.border : "rgba(255,255,255,0.06)"}`,
                    transition:"all 0.2s ease",
                  }}
                >
                  <div style={{ fontSize:"10px", fontWeight:"600", letterSpacing:"0.04em", color: isActive ? seg.c.lit : seg.c.base, transition:"color 0.2s" }}>
                    {seg.label}
                  </div>
                  <div style={{ fontFamily:"'Inter',sans-serif", fontSize:"9px", color:"rgba(148,165,185,0.3)", marginTop:"2px" }}>
                    {seg.sub}
                  </div>
                </div>
              );
          ...
          {activeSegment !== null && (
            <div style={{
              ...
              background: jwtSegmentInfo[activeSegment].c.bg,
              border: `1px solid ${jwtSegmentInfo[activeSegment].c.border}`,
              borderRadius:"6px", animation:"revealBox 0.2s ease both",
            }}>
              <p style={{ fontSize:"10px", color: jwtSegmentInfo[activeSegment].c.base, marginBottom:"8px", letterSpacing:"0.08em" }}>
                // decoded {jwtSegmentInfo[activeSegment].label.toLowerCase()}
              </p>
              {jwtSegmentInfo[activeSegment].detail.map(([k, v]) => (
                <div key={k} style={{ display:"flex", gap:"14px", lineHeight:"1.9" }}>
                  <span style={{ fontSize:"11px", color: jwtSegmentInfo[activeSegment].c.base, minWidth:"50px" }}>{k}:</span>
                  <span style={{ fontSize:"11px", color:"rgba(180,195,210,0.55)" }}>{v}</span>
                </div>
              ))}
              {jwtSegmentInfo[activeSegment].note && (
          ...
          {[
            {
              ...
              items:["⏱ Short-lived (15 min)","Sent with every API request","Store in memory or httpOnly cookie"],
            ...
            },
            {
              ...
              items:["📅 Long-lived (7 days)","Gets new access tokens","httpOnly cookie only!"],
            ...
            <div key={t.label} style={{ padding:"14px", background:t.bg, border:`1px solid ${t.border}`, borderRadius:"8px" }}>
              ...
              <p style={{ fontFamily:"'Inter',sans-serif", fontSize:"11px", color:"rgba(148,165,185,0.38)", lineHeight:"1.8" }}>
                {t.items[0]}<br/>{t.items[1]}<br/>
                <code style={{ color:t.codeColor, fontFamily:"'JetBrains Mono',monospace", fontSize:"10px" }}>{t.items[2]}</code>
              </p>
            </div>
          ))}
        </div>
